...
import com.ray.framework.util.ReflectHelper;
...
public class PaginationPlugin implements Interceptor {
		...
		if(ivk.getTarget() instanceof RoutingStatementHandler){
			...
			if(mappedStatement.getId().matches(pageSqlId)){ //拦截需要分页的SQL
				...
				if(parameterObject==null){
					...
					//replaced as count sql to improve performance
					String countSql = SqlReplaceUtil.replaceSQL(sql);
	...
	private String generatePageSql(String sql,Page page){
		if(page!=null && MybatisUtil.notEmpty(dialect)){
	...
	public void setProperties(Properties p) {
		...
		if (MybatisUtil.isEmpty(dialect)) {
			try {
		...
		if (MybatisUtil.isEmpty(pageSqlId)) {
			try {
